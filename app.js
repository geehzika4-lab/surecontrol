...
async function repairWholeOddBoostEntries({syncLinked=true}={}){
  ...
  for(const entry of APP.entries){
    const next=rebuildWholeOddSurebet(entry)||recalcSavedSure(entry);
    if(next)changed.push(next);
  }
  if(!changed.length)return 0;
  for(const next of changed){
    APP.entries=APP.entries.map(e=>e.id===next.id?next:e);
    await dbPut('entries',next);
    if(syncLinked&&next.freebetSourceId)await syncFreebetWithSurebet(next);
  }
...
function calcSureMargin(legs){
  if(!Array.isArray(legs)||!legs.length)return null;
  if(legs.some(l=>l.type==='lay'||l.freebet))return null;
  const implied=legs.reduce((sum,l)=>{
    const netOdd=1+(l.effectiveOdd-1)*(1-l.commission);
    return sum+(netOdd>1?1/netOdd:0);
  },0);
  return (1-implied)*100;
}
function recalcSavedSure(e){
  if(e?.type!=='surebet')return null;
  const raw=(e.outcomes?.length?e.outcomes:e.offers)||[];
  if(raw.length<2)return null;
  const legs=raw.map((o,i)=>({
    ...o,
    i,
    target:Number.isInteger(o.target)?o.target:i,
    odd:parseNum(o.odd||o.effectiveOdd),
    boost:Math.max(0,parseNum(o.boost)),
    commission:Math.max(0,parseNum(o.commission))/100,
    type:o.type==='lay'?'lay':'back',
    freebet:!!o.freebet,
    stake:cents(parseNum(o.stake)),
    effectiveOdd:boostedOdd(parseNum(o.odd||o.effectiveOdd),parseNum(o.boost))
  }));
  if(legs.some(l=>!(l.odd>1)))return null;

  const stakes=legs.map(l=>l.stake);
  const profits=Array.from({length:legs.length},(_,out)=>
    legs.reduce((sum,l,i)=>sum+unitPayoff(l,out)*stakes[i],0)
  );

  const grossProfit=Math.min(...profits);
  const cashUsed=legs.reduce((sum,l,i)=>sum+unitCost(l)*stakes[i],0);
  const promo=e.promotionCostApplied?parseNum(e.promotionCost):0;
  const investment=cashUsed+promo;
  const profit=grossProfit-promo;
  const roi=investment?profit/investment*100:0;

  const outcomes=legs.map((l,i)=>({
    ...l,
    commission:l.commission*100,
    stake:stakes[i],
    liability:l.type==='lay'?(l.effectiveOdd-1)*stakes[i]:0
  }));
  ...
  return {
    ...
    outcomes,
    offers:outcomes,
    coverageInvestment:cashUsed,
    investment,
    grossProfit,
    profit,
    roi,
    returnValue:investment+profit,
    outcomeProfits:profits,
    margin:calcSureMargin(legs),
    financialFormulaVersion:4,
